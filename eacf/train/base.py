...
def setup_padded_reshaped_data(data: chex.ArrayTree,
    ...
    if reshape_axis == 0:  # Used for pmap.
        test_data_reshaped, mask = jax.tree.map(
...
def batchify_data(data: chex.ArrayTree, batch_size: int):
    return jax.tree.map(lambda x: batchify_array(x, batch_size), data)
...
def eval_fn(
    ...
    if eval_on_test_batch_fn is not None:
        ...
        if isinstance(batched_info, dict):
            ...
            info.update(jax.tree.map(lambda x: jnp.sum(per_batch_weighting*x), batched_info))
            flat_mask, further_info = None, None
        else:
            per_batch_weighting = jnp.sum(mask_batched, axis=-1) / jnp.sum(jnp.sum(mask_batched, axis=-1))
            info.update(jax.tree.map(lambda x: jnp.sum(per_batch_weighting * x), batched_info[1]))
            flat_mask, further_info = jax.tree.map(lambda x: x.reshape(x.shape[0]*x.shape[1],
